...
public class Main {
    public static void main(String[] args) {
        ...
        Map<String, String> personBirthdayList = new HashMap<>();

        //"{FirstName} {LastName}" or "day/month/year"
        String nameOrBirthdate = scanner.nextLine();

        Person tempPerson = new Person(nameOrBirthdate);

        if (nameOrBirthdate.contains("/")) {
            personList.put(String.valueOf(tempPerson.getId()), tempPerson);
            personBirthdayList.put(nameOrBirthdate, String.valueOf(tempPerson.getId()));
        } else {
            personList.put(nameOrBirthdate, tempPerson);
        }

        String predecessorsInfo;
        while (!"End".equals(predecessorsInfo = scanner.nextLine())) {
            String[] token = predecessorsInfo.split("\\s+");
        ...
        if (nameOrBirthdate.contains("/")) {
            System.out.println(personList.get(personBirthdayList.get(nameOrBirthdate)));
        } else {
            System.out.println(personList.get(nameOrBirthdate));
        }
    }
}
